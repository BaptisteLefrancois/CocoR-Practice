...
public class Scanner {
	...
	static {
		...
		for (int i = 13; i <= 13; ++i) start.set(i, 9);
		start.set(44, 5); 
		start.set(43, 6); 
		start.set(40, 7); 
		start.set(41, 8); 
		start.set(Buffer.EOF, -1);
		literals.put("home", new Integer(8));
		literals.put("office", new Integer(9));
		literals.put("mobile", new Integer(10));
		literals.put("0", new Integer(11));
		...
		loop: for (;;) {
			switch (state) {
				...
				case 1:
					...
					if (ch >= 'A' && ch <= 'Z' || ch >= 'a' && ch <= 'z') {AddCh(); state = 2; break;}
